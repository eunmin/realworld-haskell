...
import RealWorld.Infra.Component.Database (State)
import qualified RealWorld.Infra.Component.Database as Database
import Relude hiding (Reader, State, ask)

type QueryDatabase es = (Reader Database.State :> es, IOE :> es)
...
getCurrentUser :: (QueryDatabase es) => GetCurrentUserParams -> Eff es (Maybe User)
getCurrentUser params = do
  (Database.State pool _) <- ask @State
...
getProfile :: (QueryDatabase es) => GetProfileParams -> Eff es (Maybe Profile)
...
listArticles :: (QueryDatabase es) => ListArticlesParams -> Eff es ArticleList
...
feedArticles :: (QueryDatabase es) => FeedArticlesParams -> Eff es ArticleList
...
getArticle :: (QueryDatabase es) => GetArticleParams -> Eff es (Maybe Article)
...
getComments :: (QueryDatabase es) => GetCommentsParams -> Eff es CommentList
...
getTags :: (QueryDatabase es) => Eff es TagList
getTags = do
  (Database.State pool _) <- ask @State
  liftIO $ withResource pool $ \conn -> do
    fromMaybe (TagList [])
